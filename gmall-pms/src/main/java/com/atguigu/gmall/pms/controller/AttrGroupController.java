...
public class AttrGroupController {
    ...
    public Resp<List<AttrGroupVO>> queryByCid(@PathVariable("catId") Long catId){
        List list=attrGroupService.queryByCid(catId);
        return Resp.ok(list);
    }

    @ApiOperation("根据分组id查询分组及组下的规格参数")
    @GetMapping("withattr/{gid}")
    public Resp queryById(@PathVariable("gid")Long catId){
        AttrGroupVO attrGroupVO = this.attrGroupService.queryById(catId);
        System.out.println(attrGroupVO.toString());
